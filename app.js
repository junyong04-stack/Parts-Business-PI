...
// ---------- chatter (미팅 중 요청사항/메모) ----------

const chatterCol = collection(db, "chatterNotes");
const chatterQuery = query(chatterCol, orderBy("createdAt", "asc"));

const chatterPanel = el("#chatter-panel");
const chatterToggle = el("#chatter-toggle");
const chatterCaret = chatterToggle.querySelector(".chatter-caret");
const chatterCount = el("#chatter-count");
const chatterMessages = el("#chatter-messages");
const chatterForm = el("#chatter-form");
const chatterAuthor = el("#chatter-author");
const chatterText = el("#chatter-text");

chatterAuthor.value = localStorage.getItem("chatterAuthor") || "";

chatterToggle.addEventListener("click", () => {
  const open = chatterPanel.classList.toggle("open");
  chatterToggle.setAttribute("aria-expanded", open ? "true" : "false");
  chatterCaret.textContent = open ? "▾" : "▴";
  if (open) chatterText.focus();
});

function formatChatterTime(ts) {
...
function renderChatter() {
  chatterCount.textContent = chatterNotes.length;
  chatterMessages.innerHTML = chatterNotes.length
    ? chatterNotes
        .map(
          (n) => `
      <div class="chatter-msg">
        <div class="chatter-msg-head">
          <span class="chatter-msg-author">${escapeHtml(n.author || "익명")}</span>
          <span class="chatter-msg-time">${formatChatterTime(n.createdAt)}</span>
          <button type="button" class="chatter-msg-del" data-id="${escapeHtml(n.id)}" title="삭제">×</button>
        </div>
        <div class="chatter-msg-text">${escapeHtml(n.text)}</div>
      </div>`
        )
        .join("")
    : `<p class="empty-note">아직 등록된 요청사항이 없습니다.</p>`;
  chatterMessages.scrollTop = chatterMessages.scrollHeight;
  chatterMessages.querySelectorAll(".chatter-msg-del").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        await deleteDoc(doc(db, "chatterNotes", btn.dataset.id));
      } catch (err) {
        console.error(err);
...
onSnapshot(
  chatterQuery,
  (snap) => {
    chatterNotes = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    renderChatter();
  },
  (err) => {
    console.error(err);
    chatterPanel.classList.add("open");
    chatterToggle.setAttribute("aria-expanded", "true");
    chatterCaret.textContent = "▾";
    chatterMessages.innerHTML = `<p class="chatter-error">요청사항 목록을 불러오지 못했습니다 (Firestore 권한 오류일 수 있습니다). 콘솔(F12)에서 자세한 오류를 확인하세요.</p>`;
  }
);
...
chatterForm.addEventListener("submit", async (e) => {
  ...
  if (!text) return;
  const author = chatterAuthor.value.trim() || "익명";
  localStorage.setItem("chatterAuthor", author);
  chatterText.value = "";
  try {
    await addDoc(chatterCol, { author, text, createdAt: serverTimestamp() });
  } catch (err) {
    console.error(err);
    alert("등록에 실패했습니다. 네트워크 연결을 확인해주세요.");
  }
});
